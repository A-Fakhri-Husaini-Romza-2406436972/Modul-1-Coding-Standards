...
class PaymentControllerTest {
    ...
    void paymentDetailByIdShouldReturnPaymentDetailView() throws Exception {
        Payment payment = createPayment("payment-2", "VOUCHER_CODE", STATUS_SUCCESS);
        when(paymentService.getPayment("payment-2")).thenReturn(payment);

        mockMvc.perform(get("/payment/detail/payment-2"))
                .andExpect(status().isOk())
                .andExpect(view().name("PaymentDetail"))
                .andExpect(model().attributeExists(PAYMENT_ATTRIBUTE))
                .andExpect(model().attribute(PAYMENT_ATTRIBUTE, samePropertyValuesAs(payment)));
    }

    @Test
    void paymentAdminListShouldReturnAllPayments() throws Exception {
        Payment firstPayment = createPayment("payment-3", "VOUCHER_CODE", STATUS_SUCCESS);
        Payment secondPayment = createPayment("payment-4", "BANK_TRANSFER", STATUS_REJECTED);
    ...
    void paymentAdminDetailShouldReturnPaymentAdminDetailView() throws Exception {
        Payment payment = createPayment("payment-5", "BANK_TRANSFER", STATUS_SUCCESS);
    ...
    void paymentSetStatusShouldCallServiceAndRedirectToAdminDetail() throws Exception {
        Payment payment = createPayment("payment-6", "VOUCHER_CODE", STATUS_SUCCESS);
        when(paymentService.getPayment("payment-6")).thenReturn(payment);
        when(paymentService.setStatus(payment, STATUS_REJECTED)).thenReturn(payment);

        mockMvc.perform(post("/payment/admin/set-status/payment-6")
                        .param("status", STATUS_REJECTED))
                .andExpect(status().is3xxRedirection())
                .andExpect(view().name("redirect:/payment/admin/detail/payment-6"));

        verify(paymentService).getPayment("payment-6");
        verify(paymentService).setStatus(eq(payment), eq(STATUS_REJECTED));
    }
